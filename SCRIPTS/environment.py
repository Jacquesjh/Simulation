...
import numpy as np
import random
from population import Person, Susceptible, Infected, normal #, Immune, Pacient, Dead
...
class House:
    ...
    def generate_house_members(self):
        ...
        for i in range(self.num_members):
            self.members.append(Susceptible(Person(self.wealth, self)))
...
class Industry:
    ...
    def __init__(self, color):
        ...
        self.num_workers  = int(np.random.normal(loc   = 2000,
                                                scale = 0.23*2000))
        self.workers      = []        
        self.home_office  = normal(mean = 0.3, std = 0.23)
        self.interactions = []
